...
async function seed() {
  try {
    ...
    if (Array.isArray(data.drinks)) {
      const result = await drinksCollection.insertMany(data.drinks);
      console.log(`Inserted ${result.insertedCount} drinks.`);
    } else {
      console.error('No drinks array found in data.json');
    }
  } catch (err) {
    console.error(err);
  } finally {
